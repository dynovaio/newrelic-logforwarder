...
const NR_DECORATION_PROPERTIES = [
    "entity.guid",
    "entity.name",
    "entity.type",
    "hostname",
...
function processLog (log, context) {
    ...
    if (properties !== undefined) {
        if (typeof properties === 'string') {
            try {
                ...
                console.warn('Can not parse properties to JSON');
            }
        }
        ...
        if (typeof properties === 'object' && properties !== null) {
            ...
            try {
                ...
                console.warn('Can not parse properties.message to JSON');
            }
            ...
            if (typeof properties.message === 'object' && properties.message !== null) {
                ...
                let newrelicDecorationProperties = Object.fromEntries(
                    Object.entries(otherProperties).filter(
                        (item) => NR_DECORATION_PROPERTIES.includes(item[0])
                    )
                );
            ...
            context.info('Structured log: ', JSON.stringify(structuredLog));
        ...
        if (parsedLogs[0].records !== undefined) {
            ...
            parsedLogs.forEach((message) => {
                message.records.forEach((log) => {
                    buffer.push(processLog(log, context));
                });
            });
            return buffer;
        } // type JSON array
